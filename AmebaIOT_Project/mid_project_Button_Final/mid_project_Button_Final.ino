...
void setup() {
  ...
  kk=0;
  while (status != WL_CONNECTED && kk<3) {
    kk++;
  ...
  if (client) {
    ...
    while (client.connected()) {    
      ...
      if (client.available()) {
        ...
        client.print("</th></tr>"); 
        client.print("<tr>");
        client.print("<td colspan=4>");
        client.print("<form method=get><input type=hidden name=L5 value=1 width:200px>"); 
        client.print("<input type=submit value=disconnect  style=width:150px></form></td>");
        client.print("</tr>"); 
        client.print("</table>");
        ...
        if (currentLine.endsWith("L1=1") && ini==0) {ini=1;count1++;} 
        else if (currentLine.endsWith("L1=0") && ini==0) {ini=1;count1--;}
        else if (currentLine.endsWith("L2=1") && ini==0) {ini=1;count5++;} 
        else if (currentLine.endsWith("L2=0") && ini==0) {ini=1;count5--;}
        else if (currentLine.endsWith("L3=1") && ini==0) {ini=1;count10++;}                  
        else if (currentLine.endsWith("L3=0") && ini==0) {ini=1;count10--;}
        else if (currentLine.endsWith("L4=1") && ini==0) {ini=1;count50++;}
        else if (currentLine.endsWith("L4=0") && ini==0) {ini=1;count50--;}
        else if (currentLine.endsWith("L5=1") && ini==0){
          ini=1;
          State=0;
  lcd.clear();
  lcd.setCursor ( 0, 0 );        // go to home
